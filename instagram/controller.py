...
class InstagramManager:
    ...
    def _test_url_for_instagram(self, url):
        """
        Test if URL is accessible for Instagram with proper headers
        """
        try:
            # Test with Instagram-like user agent
            headers = {
            ...
            if response.status_code == 200 and is_image:
                ...
                print(f"✗ URL validation failed: Status {response.status_code}, Content-Type: {content_type}")
                return False
    ...
    def _upload_to_cloudinary(self, image_path):
        ...
        try:
            ...
            # For Cloudinary, we need to create signature without api_key
            # Only include parameters that will be sent in the request
            params_to_sign = {
                'timestamp': timestamp
            }
            ...
            # Add the API secret at the end for signature
            signature_string = params_string + self.cloudinary_api_secret
